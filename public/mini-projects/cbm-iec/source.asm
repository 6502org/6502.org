; Enhanced BASIC to assemble under Tass, $ver 1.10
; Lee Davidson http://mycorner.no-ip.org/6502/ehbasic/index.html
;
;*******************************************************************
; four changes made for compatibility with the SBC Operating System:
; 1) EhBASIC starts at $B000 rather that $C000, to make room for future growth
; 2) rambase changed from $0300 to $0400, to preserve the Monitor's buffers
; 3) ramtop changed from $C000 to $7F00 (Physical RAM is $0000-$7EFF)
; 4) instruction "stx  ibuffs-1" added shortly after label LAB_2D13,
;    forcing a requirement documented by Lee.
;*******************************************************************
;
; 2003/05/16  RAC -- Patched LAB_SCGB routine
; 2003/05/19  RAC -- Synched with SBCOS v2.5
; 2003/07/02  RAC -- Synched with EhBASIC 1.05
; 2003/09/05  RAC -- Synched with SBCOS v2.5a 8/19/03
...
;*** changed by Daryl Rictor for SBC compatibility from $0300 to $0400 ***
Ram_base		= $0400	; start of user RAM (set as needed, should be page aligned)
;*** changed by Daryl Rictor for SBC compatibility from $C000 to $7F00 ***
Ram_top		= $7F00	; end of user RAM+1 (set as needed, should be page aligned)
;Ram_top		= $8000	; end of user RAM+1 for CUBE


;*** changed by Daryl Rictor for SBC compatibility from $C000 to $B000 ***
;	*=	$B000
;	*=	$8000			; for CUBE
...
LAB_2D13
	...
	LDX	#$FF			; set byte
	stx   ibuffs-1		; *** added by Daryl Rictor for SBC compatibility***
...
LAB_1785
	BCS	LAB_177F		; return if carry set (end of scan, character was not 0-9)
...
LAB_1C56
	...
	JMP	LAB_CTNM		; check if source is numeric & RTS, else do type mismatch
					; (string functions avoid this by dumping return address)
...
LAB_SMSG
	...
	.byte	"Enhanced BASIC 1.10",$0A,$00
